// Netlify Function to handle donation form submission to Airtable with Resend email notifications
...
exports.handler = async (event) => {
    ...
    try {
        ...
        if (!formData.nom || !formData.email || !formData.montant || !formData.paiement || !formData.gdpr) {
            return {
                statusCode: 400,
                headers: corsHeaders,
        ...
        if (isNaN(montant) || montant < 1 || montant > 4600) {
            return {
                statusCode: 400,
                headers: corsHeaders,
                body: JSON.stringify({ error: 'Montant invalide (1-4600€)' })
            };
        }
        ...
        // Real cost after tax reduction
        const realCost = Math.round(montant * 0.34);

        // Build Message field with [DON] prefix
        let fullMessage = `[DON] Montant : ${montant}€ (coût réel : ${realCost}€) | Paiement : ${formData.paiement}`;
        if (formData.message) {
            fullMessage += `\n\n--- Message ---\n${formData.message}`;
        }
        ...
        if (RESEND_API_KEY) {
            try {
                ...
                const emailStyles = `
                    ...
                    .highlight-box { background-color: #edf7ed; border-left: 4px solid #6cb13e; padding: 15px; margin: 20px 0; }
                    ...
                    .amount-badge { display: inline-block; background-color: #6cb13e; color: white; padding: 5px 15px; border-radius: 15px; font-size: 14px; margin-bottom: 10px; }
                `;

                // 1. Acknowledgment email to the donor
                ...
                <body>
                    <div class="container">
                        ...
                        <div class="content">
                            ...
                            <p>Merci pour votre soutien à notre campagne !</p>

                            <div class="highlight-box">
                                <strong>Votre demande de don de ${montant} € a été enregistrée le ${formattedDate}</strong><br>
                                <span class="green-accent">Coût réel après réduction d'impôt : ${realCost} €</span>
                            </div>

                            <p>Notre équipe vous contactera dans les meilleurs délais avec les instructions de paiement (${formData.paiement.toLowerCase()}).</p>

                            <div class="message-box">
                                <span class="amount-badge">Don : ${montant} €</span>
                                <div class="info-row">
                                    <span class="info-label">Mode de paiement :</span> ${formData.paiement}
                                </div>
                                <div class="info-row">
                                    <span class="info-label">Réduction d'impôt :</span> ${Math.round(montant * 0.66)} € (66%)
                                </div>
                            </div>

                            <p>Un <strong>reçu fiscal</strong> vous sera délivré pour votre déclaration d'impôts.</p>
                ...
                <head>
                    <meta charset="UTF-8">
                    <meta name="viewport" content="width=device-width, initial-scale=1.0">
                    <title>Nouvelle demande de don</title>
                    <style>${emailStyles}</style>
                </head>
                <body>
                    <div class="container">
                        <div class="header">
                            <h1>Nouvelle demande de don</h1>
                        </div>
                        <div class="content">
                            <h2>Un nouveau don a été proposé</h2>
                            ...
                            <div class="message-box">
                                <span class="amount-badge">Don : ${montant} € → Coût réel : ${realCost} €</span>
                                <div class="info-row">
                                ...
                                ${formData.phone ? `<div class="info-row"><span class="info-label">Téléphone :</span> ${formData.phone}</div>` : ''}
                                <div class="info-row">
                                    <span class="info-label">Montant :</span> ${montant} €
                                </div>
                                <div class="info-row">
                                    <span class="info-label">Mode de paiement :</span> ${formData.paiement}
                                </div>
                            ...
                            <a href="mailto:${formData.email}?subject=Votre don de ${montant}€ - Pour Senlis en Confiance" class="btn" style="display: inline-block; background-color: #6cb13e; color: #ffffff !important; padding: 12px 30px; text-decoration: none; border-radius: 25px; font-weight: bold;">Répondre à ${formData.nom.split(' ')[0]}</a>
                        </div>
                        <div class="footer">
                ...
                await fetch('https://api.resend.com/emails', {
                    ...
                    body: JSON.stringify({
                        ...
                        to: [formData.email],
                        subject: 'Merci pour votre soutien - Pour Senlis en Confiance',
                ...
                await fetch('https://api.resend.com/emails', {
                    ...
                    body: JSON.stringify({
                        ...
                        subject: `Nouveau don de ${formData.nom} - ${montant}€`,
        ...
        return {
            ...
            body: JSON.stringify({
                success: true,
                message: 'Votre demande de don a été enregistrée avec succès',
