// Test endpoint for daily stats - manually trigger to test
const PLAUSIBLE_API_KEY = process.env.PLAUSIBLE_API_KEY;
const RESEND_API_KEY = process.env.RESEND_API_KEY;
const SITE_ID = 'poursenlisenconfiance.fr';

exports.handler = async (event, context) => {
    console.log('Test stats function triggered');
    console.log('PLAUSIBLE_API_KEY exists:', !!PLAUSIBLE_API_KEY);
    console.log('RESEND_API_KEY exists:', !!RESEND_API_KEY);

    if (!PLAUSIBLE_API_KEY) {
        return {
            statusCode: 500,
            body: JSON.stringify({ error: 'Missing PLAUSIBLE_API_KEY' })
        };
    }
    ...
    try {
        ...
        const statsUrl = `https://plausible.io/api/v1/stats/aggregate?site_id=${SITE_ID}&period=day&date=${dateStr}&metrics=visitors,pageviews,bounce_rate,visit_duration`;
        ...
        if (!statsResponse.ok) {
            ...
            console.error('Plausible error:', errorText);
            return {
                statusCode: 500,
                body: JSON.stringify({ error: `Plausible API error: ${statsResponse.status}`, details: errorText })
            };
        }
        ...
        if (!emailResponse.ok) {
            ...
            return {
                statusCode: 500,
                body: JSON.stringify({ error: `Resend API error: ${emailResponse.status}`, details: errorText })
            };
        }
